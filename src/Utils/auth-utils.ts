...
async function handlePreKeyOperations(
	...
	await mutex.runExclusive(async () => {
		const keyData = data[keyType as keyof SignalDataTypeMap]
		if (!keyData) return

		// Ensure structures exist
		transactionCache[keyType as keyof SignalDataTypeMap] = transactionCache[keyType as keyof SignalDataTypeMap] || ({} as any)
		mutations[keyType as keyof SignalDataTypeMap] = mutations[keyType as keyof SignalDataTypeMap] || ({} as any)
		...
		for (const keyId of updateKeys) {
			;(transactionCache[keyType as keyof SignalDataTypeMap] as any)[keyId] = keyData[keyId]
			;(mutations[keyType as keyof SignalDataTypeMap] as any)[keyId] = keyData[keyId]
		}
		...
		if (isInTransaction) {
			...
			for (const keyId of deletionKeys) {
				if ((transactionCache[keyType as keyof SignalDataTypeMap] as any)?.[keyId]) {
					;(transactionCache[keyType as keyof SignalDataTypeMap] as any)[keyId] = null
					;(mutations[keyType as keyof SignalDataTypeMap] as any)[keyId] = null
				} else {
...
function handleNormalKeyOperations(
	...
	Object.assign(transactionCache[key as keyof SignalDataTypeMap] as any, data[key as keyof SignalDataTypeMap])
	mutations[key as keyof SignalDataTypeMap] = mutations[key as keyof SignalDataTypeMap] || ({} as any)
	Object.assign(mutations[key as keyof SignalDataTypeMap] as any, data[key as keyof SignalDataTypeMap])
}
	...
	await mutex.runExclusive(async () => {
		...
		for (const keyId in keyData) {
			if (keyData[keyId] === null) {
				...
				if (!existingKeys[keyId]) {
					logger.warn(`Skipping deletion of non-existent ${keyType}: ${keyId}`)
					delete (data[keyType as keyof SignalDataTypeMap] as any)[keyId]
				}
			}
		}
	})
					...
					for (const senderKeyName of senderKeyNames) {
						await queueSenderKeyOperation(senderKeyName, async () => {
							...
							const senderKeyData: SignalDataSet = {
								'sender-key': {
									[senderKeyName]: data['sender-key']?.[senderKeyName] || null								}
							}
